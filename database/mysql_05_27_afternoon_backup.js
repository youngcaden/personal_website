...
*/
function handleDisconnection_conn(){
       ...
       if(err){
           throw err;
           setTimeout('handleDisconnection_conn()', 2000);
       }
       });
       conn.on('error', function(err) {
          console.log(err);
          if(err.code === 'PROTOCOL_CONNECTION_LOST') {
                handleDisconnection_conn();
          }
       });
}

function handleDisconnection_con(){
       ...
       if(err){
           throw err;
           setTimeout('handleDisconnection_con()', 2000);
       }
       });
       con.on('error', function(err) {
            console.log(err);
            if(err.code === 'PROTOCOL_CONNECTION_LOST') {
                  handleDisconnection_con();
            }
       });

}

function handleDisconnection_con_admin(){
        con_admin = mysql.createConnection({   
        host: "localhost",
        user: "root",
        password: "root",
       ...
       if(err){
           throw err;
           setTimeout('handleDisconnection_con_admin()', 2000);
       }
       });
       con_admin.on('error', function(err) {
          console.log(err);
          if(err.code === 'PROTOCOL_CONNECTION_LOST') {
                handleDisconnection_con_admin();
          }
       });
}
...
module.exports = function(event){      // in file uiCommunications.js, var dbController = new dbAccess(event_dbAccess);
  ...
  conn = mysql.createConnection({    //create conn to connect the database db which includes two tables(car,lift)
  host: "localhost",
  user: "root",
  password: "root",
  database:"db"
...
conn.connect(function(err) {
  if(err) throw err;
  else console.log("MySQL-Database db Connected!");
});
///////////////////////////////////////////////////////////////////////////////added on 25/05/2018
conn.on('error', function(err) {
    console.log(err);
  ...
  con = mysql.createConnection({     //create con to connect the database logs which includes two tables(car_log,lift_log)
  host: "localhost",
  user: "root",
  password: "root",
  database:"logs"
...
con.connect(function(err) {
  if(err) throw err;
  else console.log("MySQL-Database logs Connected!");
});
///////////////////////////////////////////////////////////////////////////////added on 25/05/2018
con.on('error', function(err) {
    console.log(err);
  ...
  con_admin = mysql.createConnection({     //create con_admin to connect the database administrartors which includes table admin
  host: "localhost",
  user: "root",
  password: "root",
  database:"administrators"
...
con_admin.connect(function(err) {
  if(err) throw err;
  else console.log("MySQL-Database administrators Connected!");
});
///////////////////////////////////////////////////////////////////////////////added on 25/05/2018
con_admin.on('error', function(err) {
    console.log(err);
...
*/
   ...
   for(var i = 0; i < data.length; i++){
         if(data[i].length !== 6){
 	    console.log(data[i], i, 'is invalid');
	    continue;	
	 }
         if(data[i][3] == 'make' || data[i][4] == 'model' || data[i][5] == 'year'){
 	    console.log(data[i], i, 'is invalid');
	    continue;	
         }

	  var recordJSON = {make:'',model:'',year:'',front:0,rear:0,leng:0};
	  recordJSON.make = data[i][3];
	  recordJSON.model = data[i][4];
	  recordJSON.year = data[i][5];
	  recordJSON.front = data[i][0];
	  recordJSON.rear = data[i][1];
	  recordJSON.leng = data[i][2];
	  try{
...
*/
   ...
   for(var i = 0; i < data.length; i++){
		...
		continue;	
          ...
          if(data[i][0] == 'lift_name'){
 	        console.log(data[i], i, 'is invalid');
		continue;	
	  }

	  var recordJSON = {lift_name:'',WT:'', PD:'', LCF:0, LCR:0, RFE:0, RFR:0, RRE:0, RRR:0, DAP:0, YO:0, WOF:0, WOR:0};
	  recordJSON.lift_name = data[i][0];
	  recordJSON.WT = data[i][1];
	  recordJSON.PD = data[i][2];
	  recordJSON.LCF = data[i][3];
	  recordJSON.LCR = data[i][4];
	  recordJSON.RFE = data[i][5];
	  recordJSON.RFR = data[i][6];
	  recordJSON.RRE = data[i][7];
	  recordJSON.RRR = data[i][8];
	  recordJSON.DAP = data[i][9];
	  recordJSON.YO = data[i][10];
	  recordJSON.WOF = data[i][11];
	  recordJSON.WOR = data[i][12];
